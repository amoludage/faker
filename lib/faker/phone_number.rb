module Faker
  class PhoneNumber < Base
    class << self
      def phone_number
        parse('phone_number.formats') == "" ? numerify(fetch('phone_number.formats')) : parse('phone_number.formats')
      end

      def cell_phone
        parse('cell_phone.formats') == "" ? numerify(fetch('cell_phone.formats')) : parse('cell_phone.formats')
      end
